    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Uploaded Images for Instant Mode */}
          {detail.video.workflow?.mode === 'instant' && detail.video.uploadedImages && detail.video.uploadedImages.length > 0 && (
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-4 sm:p-6">
                <h3 className="text-lg font-semibold text-gray-100 mb-4">アップロードされた画像</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {detail.video.uploadedImages.map((imageUrl, index) => (
                    <div key={index} className="bg-gray-800 rounded-lg p-3">
                      ...
                      <p className="text-sm font-medium text-gray-100 text-center">
                        画像 {index + 1}
                      </p>
                    </div>
                  ))}
          ...
          {/* Detected Faces for Instant Mode */}
          {detail.video.workflow?.mode === 'instant' && detail.video.detectedFaces && detail.video.detectedFaces.length > 0 && (
            <Card className="bg-gray-900 border-gray-800">
              <CardContent className="p-4 sm:p-6">
                <h3 className="text-lg font-semibold text-gray-100 mb-4">検出された顔画像</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {detail.video.detectedFaces.map((face) => (
                    <div key={face.id} className="bg-gray-800 rounded-lg p-3">
                      <div className="aspect-square relative overflow-hidden rounded-lg mb-2 bg-gray-700">
                        <img
                          src={face.face_image_url}
                          alt={`${face.tag_name || face.name || '未設定'}の顔`}
                          className="absolute inset-0 w-full h-full object-cover"
                          onError={(e) => {
                            const target = e.target as HTMLImageElement;
                            target.style.display = 'none';
                      ...
                      <p className="text-sm font-medium text-gray-100 text-center truncate">
                        {face.tag_name || face.name || '未設定'}
                      </p>
                    </div>
                  ))}
                </div>
